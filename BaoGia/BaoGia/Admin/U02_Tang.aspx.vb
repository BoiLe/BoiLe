...
Public Class U02_Tang
                        ...
                        Using connection As OleDbConnection = New OleDbConnection(excelConnectionString)
                            ...
                            Using dr As DbDataReader = command.ExecuteReader()
                                ...
                                Using bulkCopy As SqlBulkCopy = New SqlBulkCopy(sqlConnectionString)
                                    bulkCopy.DestinationTableName = "tbl15_Tang"
    ...
    Protected Sub btnAddRecord_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If IsValid Then
            Dim parameters = SqlDataSource1_Tang.InsertParameters
            parameters("TenTang").DefaultValue = txtTenTang.Text
            parameters("MoTa").DefaultValue = txtMoTa.Text
            parameters("MaDuAn").DefaultValue = Session("MaDuAn").ToString() 'ddlMaDuAn.SelectedValue

            Try

                If txtTenTang.Text = "" Then
                    ScriptManager.RegisterStartupScript(Me, [GetType](), "showalert", "alert('Các trường (*) không được bỏ trống.');", True)
                Else
                    SqlDataSource1_Tang.Insert()
                    txtTenTang.Text = ""
                    txtMoTa.Text = ""
                    Dim sb As StringBuilder = New StringBuilder()
                    sb.Append("<script type='text/javascript'>")
    ...
    Protected Sub GridView1_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs)
        ...
        If e.CommandName.Equals("editRecord") Then
            ...
            txtMaTang1.Text = HttpUtility.HtmlDecode(gvrow.Cells(3).Text).ToString()
            txtTenTang1.Text = HttpUtility.HtmlDecode(gvrow.Cells(4).Text).ToString().Trim()
            txtMoTa1.Text = HttpUtility.HtmlDecode(gvrow.Cells(5).Text).ToString().Trim()
            lblMaDuAn.Text = HttpUtility.HtmlDecode(gvrow.Cells(6).Text).ToString().Trim()
        ...
        ElseIf e.CommandName.Equals("selectRecord") Then
            Dim gvrow As GridViewRow = GridView1.Rows(index)
            Session("MaTang") = HttpUtility.HtmlDecode(gvrow.Cells(3).Text).ToString()
            Session("TenTang") = HttpUtility.HtmlDecode(gvrow.Cells(4).Text).ToString()
            Dim url As String = "/Admin/U03_Phong.aspx"
            Dim sb As StringBuilder = New StringBuilder()
            sb.Append("<script type = 'text/javascript'>")
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As EventArgs)
        If IsValid Then
            Dim parameters = SqlDataSource1_Tang.UpdateParameters
            parameters("MaTang").DefaultValue = txtMaTang1.Text
            parameters("TenTang").DefaultValue = txtTenTang1.Text
            parameters("MoTa").DefaultValue = txtMoTa1.Text
            parameters("MaDuAn").DefaultValue = Session("MaDuAn").ToString() 'ddlMaDuAn1.SelectedValue

            Try

                If txtMaTang1.Text = "" Then
                    ScriptManager.RegisterStartupScript(Me, [GetType](), "showalert", "alert('Các trường (*) không được bỏ trống.');", True)
                Else
                    SqlDataSource1_Tang.Update()
                    Dim sb As StringBuilder = New StringBuilder()
                    sb.Append("<script type='text/javascript'>")
    ...
    Private Sub executeDelete(ByVal code As String)
        If IsValid Then
            Dim parameters = SqlDataSource1_Tang.DeleteParameters
            parameters("MaTang").DefaultValue = code

            Try
                SqlDataSource1_Tang.Delete()
            Catch __unusedException1__ As Exception
                ScriptManager.RegisterStartupScript(Me, [GetType](), "showalert", "alert('Vui lòng kiểm tra lại.');", True)
            End Try
        End If
